...
{
    //if c is contained in word
    //if c is not contained in word
    //if c is already been guessed

    class HangmanGame
    {
        //this is the word you're trying to guess
        public string word;
        public int tries = 0; //ticks up each time you guesss
        ...
        public void Run()
        {
            while (HasWon() == false)
            {
                ...
                PlayRound(c); //feed it in to PlayRound
            }
            PrintProgress();
        }
        ...
        public void PlayRound(char guess)
        {
            ...
            else if (word.Contains(guess))
            {
                ...
                int index = word.IndexOf(guess); //find the Index of guess
                foundLetters[index] = guess;    //insert guess into foundLetterss at correct index
